CHRISTOPHER COX PROGRAM PORTFOLIO - PORTABLE HTML

Open index.html in a web browser to view the site locally.

To publish the site, upload this entire folder to a static website host. Keep the assets and downloads folders next to index.html so the logo and binder link continue to work.

Files:
- index.html: complete portable website, including styling and competency tabs
- assets/iowa-cyp-logo.png: website logo
- downloads/Chris-Cox-Competency-Portfolio-2026.pdf: downloadable competency binder
